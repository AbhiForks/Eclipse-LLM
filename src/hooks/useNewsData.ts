...
import {
  getCategoryQuery,
  getBackupNewsData,
} from "@/components/discover/NewsUtils";
      ...
      try {
        ...
        if (data.status === "ok" && data.articles) {
          const newArticles = data.articles.map(
            (article: any, index: number) => ({
              ...
              imageUrl:
                article.urlToImage ||
                `https://placehold.co/600x400/272727/ffffff?text=News`,
              date: article.publishedAt
                ? new Date(article.publishedAt).toRelativeTime()
                : "Recently",
          ...
          setNewsItems((prevItems) =>
            reset ? newArticles : [...prevItems, ...newArticles],
          );
        ...
        if (reset) {
          ...
          toast({
            ...
            description:
              "Using cached news instead. Please check your internet connection.",
            variant: "destructive",
          });
        }
